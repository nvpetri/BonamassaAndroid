# Android cliente — preparação para produção
...
```powershell
Get-FileHash "C:\releases\bonamassa-cliente-release.apk" -Algorithm SHA256
```
...
1. Login/cadastro permitido pela API, imagens e cardápio atualizado.
2. Inteira/meio a meio, borda, combo com desconto, bebida e carrinho.
3. Compra imediata com a loja aberta e reserva quando fechada.
4. Tela de confirmação deixa claro o horário; acompanhar e cancelar conforme regra.
5. Perda de rede após envio: reconciliar sem duplicar pedido.
6. Sessão expirada/revogada, reinício do app, pagamento e status exibidos corretamente.
